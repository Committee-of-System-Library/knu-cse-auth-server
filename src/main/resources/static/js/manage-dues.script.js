...
function searchDues() {
    duesSearchColumn = document.getElementById("duesSearchColumn").value; // ""면 전체
...
async function createDues() {
    ...
    try {
        const response = await fetch(`${contextPath}/manage/dues`, {
            ...
            body: JSON.stringify({ studentId: Number(studentId), depositorName, amount, remainingSemesters, submittedAt })
        });
        if (!response.ok) {
