...
function searchProvider() {
    ...
    providerSearchKeyword = document.getElementById("providerSearchKeyword").value.trim();
    providerPage = 0;
    loadProviderPage();
}
...
async function loadProviderPage() {
    try {
        const url = new URL(`${contextPath}/manage/providers`, window.location.origin);
...
function renderProviderTable(arr) {
    ...
    arr.forEach((item) => {
      ...
      <td contenteditable="true" data-field="providerName">${item.providerName}</td>
      <td contenteditable="true" data-field="providerKey">${item.providerKey}</td>
      <td contenteditable="true" data-field="studentId">${item.studentId ?? ""}</td>
...
function renderProviderPagination() {
    renderPagination("providerPagination", providerPage, providerTotalPages, (page) => {
        providerPage = page;
        loadProviderPage();
    });
}
...
async function createProvider() {
    ...
    const studentId = document.getElementById("newProviderStudentId").value.trim();
...
async function updateProvider(button) {
    ...
    const providerName = row.querySelector("[data-field='providerName']").innerText.trim();
    const providerKey = row.querySelector("[data-field='providerKey']").innerText.trim();
    const studentIdStr = row.querySelector("[data-field='studentId']").innerText.trim();
    const studentId = studentIdStr ? Number(studentIdStr) : null;

    try {
        const response = await fetch(`${contextPath}/manage/providers/${providerId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
...
async function deleteSelectedProviders() {
    const checkboxes = document.querySelectorAll("#providerTable tbody input[type='checkbox']:checked");
