...
document.addEventListener("DOMContentLoaded", () => {
    // 혹시 기본 섹션이 Student라면 불러오기
    loadStudentPage();
});

function searchStudent() {
    studentSearchColumn = document.getElementById("studentSearchColumn").value; // ""이면 전체
...
async function loadStudentPage() {
    try {
        const url = new URL(`${contextPath}/manage/students`, window.location.origin);
    ...
    students.forEach((item) => {
      ...
      <td>
        <select class="styled-dropdown" data-field="major">
          ${["PLATFORM", "AI", "GLOBAL", "NONE"].map(
            (m) => `<option value="${m}" ${item.major === m ? "selected" : ""}>${m}</option>`
        ).join("")}
        </select>
      </td>
      <td>
        <select class="styled-dropdown" data-field="role">
          ${["ROLE_STUDENT", "ROLE_EXECUTIVE", "ROLE_FINANCE", "ROLE_ADMIN"].map(
            (r) => `<option value="${r}" ${item.role === r ? "selected" : ""}>${r}</option>`
        ).join("")}
        </select>
      </td>
      <td>X</td>
...
async function updateStudent(button) {
    ...
    try {
        const response = await fetch(`${contextPath}/manage/students/${studentId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ studentNumber, name, major, role })
        });
