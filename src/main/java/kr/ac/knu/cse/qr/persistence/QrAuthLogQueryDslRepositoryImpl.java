...
public class QrAuthLogQueryDslRepositoryImpl extends QueryDslSupport implements QrAuthLogQueryDslRepository {
    ...
    public Page<QrAuthLogResponse> findQrAuthLogs(QrAuthLogSearchFilter filter, Pageable pageable) {
        ...
        if (filter.getSearchColumn() != null && !filter.getSearchColumn().isBlank() && filter.getSearchKeyword() != null && !filter.getSearchKeyword().isBlank()) {
        ...
        List<Tuple> tuples = queryFactory.select(qrAuthLog.id, qrAuthLog.scanDate, qrAuthLog.studentNumber, qrAuthLog.studentName, qrAuthLog.duesPaid, qrAuthLog.scannedBy).from(qrAuthLog).where(whereBuilder).orderBy(new OrderSpecifier<>(order, sortPath)).offset(pageable.getOffset()).limit(pageable.getPageSize()).fetch();

        List<QrAuthLogResponse> content = tuples.stream().map(t -> {
            String studentNumber = t.get(qrAuthLog.studentNumber);

            boolean actualDuesPaid = studentRepository.findByStudentNumber(studentNumber).map(student -> duesRepository.findByStudent(student).isPresent()).orElse(false);

            return new QrAuthLogResponse(t.get(qrAuthLog.id), t.get(qrAuthLog.scanDate), studentNumber, t.get(qrAuthLog.studentName), actualDuesPaid, t.get(qrAuthLog.scannedBy));
        }).toList();
        ...
        return paginate(pageable, content, countQuery -> {
            ...
            return countQuery.select(qrAuthLog.count()).from(qrAuthLog).where(wb);
        });
    }
